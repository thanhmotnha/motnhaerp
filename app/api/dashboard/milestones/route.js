...
export const GET = withAuth(async () => {
    ...
    const [commitments, workOrders, contractPayments] = await Promise.all([
        ...
        prisma.commitment.findMany({
            ...
            select: { id: true, title: true, deadline: true, project: { select: { id: true, name: true, code: true } } },
        ...
        prisma.workOrder.findMany({
            ...
            select: { id: true, code: true, title: true, dueDate: true, project: { select: { id: true, name: true, code: true } } },
            orderBy: { dueDate: 'asc' },
            take: 10,
        }),
        ...
        prisma.contractPayment.findMany({
            ...
            select: { id: true, phase: true, amount: true, dueDate: true, contract: { select: { name: true, project: { select: { id: true, name: true, code: true } } } } },
            orderBy: { dueDate: 'asc' },
            take: 10,
        }),
    ]);
    ...
    const milestones = [
        ...commitments.map(c => ({
            ...
            dueDate: c.deadline,
            href: `/projects/${c.project?.code || c.project?.id}`,
        })),
        ...workOrders.map(w => ({
            id: w.id,
            type: 'workorder',
        ...
        ...contractPayments.map(p => ({
            ...
            dueDate: p.dueDate,
            href: p.contract?.project?.code ? `/projects/${p.contract.project.code}` : '#',
        })),
