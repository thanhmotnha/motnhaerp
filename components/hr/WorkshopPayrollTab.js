...
export default function WorkshopPayrollTab() {
    ...
    const calcPreview = (rec) => {
        ...
        const netSalary = grossIncome - disciplinaryFine - salaryAdvance;
        return { basePay, overtimePay, grossIncome, netSalary };
    };
                ...
                {records.length > 0 && (
                    <>
                        ...
                        <button className="btn btn-ghost btn-sm"
                            onClick={() => window.open(`/api/hr/workshop-payroll/export?month=${month}&year=${year}`, '_blank')}>
                            📥 Excel
                        </button>
                    </>
                )}
                    ...
                    <table className="data-table" style={{ fontSize: 12, minWidth: 1100 }}>
                            ...
                            {records.map(rec => {
                                ...
                                return (
                                    <tr key={rec.id} style={{ background: isDirty ? 'rgba(var(--accent-rgb),0.02)' : undefined }}>
                                        ...
                                        <td style={tdR}>{fmt(pv?.basePay ?? rec.basePay)}</td>
                                        <td style={tdR}>{numInput(rec, 'overtimeHours', 55)}</td>
                                        <td style={tdR}>{fmt(pv?.overtimePay ?? rec.overtimePay)}</td>
                        ...
                        <tfoot>
                            <tr style={{ background: 'var(--bg-secondary)', fontWeight: 700 }}>
                                <td colSpan={11} style={{ padding: '8px 8px', textAlign: 'right', fontSize: 12 }}>
