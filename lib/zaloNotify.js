...
/**
 * NOTE: Adapted to new workshop schema (WorkshopTask → WorkshopTaskWorker many-to-many).
 * Fields bị drop: workerId, assignedById, dueDate, assignedBy/worker relations.
 * Schema mới dùng `workers` (array), `deadline` thay vì `dueDate`. Gửi Zalo cho tất cả thợ.
 * Chỉ giữ lại cho backward-compat; entry-point chính hiện tại là `notifyWorkshopTaskAssigned` trong lib/notify.js.
 */
export async function notifyTaskAssigned(taskId) {
    const task = await prisma.workshopTask.findUnique({
        where: { id: taskId },
        include: {
            workers: { include: { worker: { select: { name: true, zaloUserId: true } } } },
            project: { select: { name: true } },
        },
    });
    if (!task) return;
    const recipients = (task.workers || [])
        .map(tw => tw.worker?.zaloUserId)
        .filter(Boolean);
    if (recipients.length === 0) return;
    const due = task.deadline ? new Date(task.deadline).toLocaleDateString('vi-VN') : '—';
    ...
    const results = await Promise.all(recipients.map(zid => sendZaloToZaloId(zid, text)));
