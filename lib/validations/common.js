...
// updDate: unlike optDate which transforms undefined→null, this returns undefined for absent/empty
// so Prisma skips the field entirely instead of trying to set a non-nullable DateTime to null
export const updDate = z.string().optional().nullable().transform(v => {
...
export function safePartial(createSchema) {
    // Strip .strict() if present, then make partial — prevents ZodError on unknown keys during updates
    const base = typeof createSchema.strip === 'function' ? createSchema.strip() : createSchema;
