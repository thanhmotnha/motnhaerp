...
import { TrendingUp, TrendingDown, DollarSign, AlertTriangle, Search, RefreshCw, ExternalLink } from 'lucide-react';
...
export default function PLByProjectPage() {
    ...
    const [sortDir, setSortDir] = useState('asc'); // ascending = worst margin first
    ...
    const SortBtn = ({ field, label }) => (
        <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', fontWeight: sortField === field ? 700 : 400, display: 'flex', alignItems: 'center', gap: 2 }}
        >
            {label}
        ...
        <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
                <div>
                    ...
                    <p style={{ color: '#6b7280', fontSize: 13, margin: '4px 0 0' }}>Phân tích hiệu quả tài chính từng dự án</p>
                </div>
                <button onClick={load} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#f3f4f6', border: '1px solid #e5e7eb', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            ...
            {sum && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 24 }}>
                    ...
                    <SummaryCard icon={<TrendingDown size={20} />} label="Tổng chi phí" value={fmt(sum.totalCost)} color="#f59e0b" />
                    <SummaryCard
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ height: 36, border: '1px solid #e5e7eb', borderRadius: 8, padding: '0 10px', fontSize: 13, background: '#fff' }}>
                ...
                <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: 10 }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        ...
                        <tbody>
                            {rows.length === 0 ? (
                                <tr>
                                    <td colSpan={9} style={{ textAlign: 'center', padding: 30, color: '#9ca3af' }}>Không có dữ liệu</td>
                                </tr>
                            ) : rows.map(r => {
                                const isAlert = r.alert;
                                const rowBg = isAlert ? '#fff7ed' : (r.margin < 0 ? '#fef2f2' : '#fff');
                                return (
                                    <tr key={r.id} style={{ background: rowBg, borderTop: '1px solid #f3f4f6' }}>
                                        <td style={td}>
                                            <div style={{ fontWeight: 600, fontSize: 12.5 }}>{r.code || '—'}</div>
                                            <div style={{ color: '#374151', maxWidth: 200 }}>{r.name}</div>
                                            {isAlert && (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#b45309', fontSize: 11, marginTop: 2 }}>
                                                    <AlertTriangle size={11} /> Margin thấp
                                                </div>
                                            )}
                                        </td>
                                        <td style={td}>{r.customerName || '—'}</td>
                                        <td style={td}>
                                            <span style={{ padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: STATUS_COLOR[r.status] + '20', color: STATUS_COLOR[r.status] || '#6b7280' }}>
                                                {r.status || '—'}
                                            </span>
                                        </td>
                                        <td style={{ ...td, textAlign: 'right', fontWeight: 500 }}>{fmt(r.contractValue)}</td>
                                        <td style={{ ...td, textAlign: 'right', color: '#16a34a' }}>{fmt(r.paidByCustomer)}</td>
                                        <td style={{ ...td, textAlign: 'right', color: '#dc2626' }}>{fmt(r.totalCost)}</td>
                        ...
                        {rows.length > 0 && (
                            <tfoot style={{ background: '#f9fafb', fontWeight: 700, borderTop: '2px solid #e5e7eb' }}>
                                <tr>
                                    <td style={td} colSpan={3}>Tổng ({rows.length} dự án)</td>
                                    <td style={{ ...td, textAlign: 'right' }}>{fmt(rows.reduce((s, r) => s + r.contractValue, 0))}</td>
                                    <td style={{ ...td, textAlign: 'right', color: '#16a34a' }}>{fmt(rows.reduce((s, r) => s + r.paidByCustomer, 0))}</td>
                                    <td style={{ ...td, textAlign: 'right', color: '#dc2626' }}>{fmt(rows.reduce((s, r) => s + r.totalCost, 0))}</td>
                                    <td style={{ ...td, textAlign: 'right' }}>
                                        {(() => {
                                            const p = rows.reduce((s, r) => s + r.grossProfit, 0);
                                            return <span style={{ color: p >= 0 ? '#16a34a' : '#dc2626' }}>{p >= 0 ? '' : '-'}{fmt(Math.abs(p))}</span>;
                                        })()}
                                    </td>
            ...
            <div style={{ marginTop: 12, fontSize: 11, color: '#9ca3af' }}>
                * Lợi nhuận = Đã thu − (Chi nhà thầu + Mua hàng đã trả + Chi phí DA) &nbsp;|&nbsp; Cảnh báo: margin &lt; 10%
            </div>
...
function MarginBadge({ margin }) {
    ...
    const bg = color + '18';
    return (
        <span style={{ background: bg, color, padding: '2px 8px', borderRadius: 20, fontWeight: 700, fontSize: 12 }}>
            {margin}%
        </span>
    );
}
