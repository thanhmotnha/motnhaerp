/**
 * Upload ảnh Ván Thái lên production (R2) qua API /api/upload
 * + Cập nhật image URL trong DB
 *
 * Cách lấy SESSION_TOKEN:
 *   Mở erp.motnha.vn → F12 → Application → Cookies → next-auth.session-token
 *
 * Run: SESSION_TOKEN=xxx node scripts/upload-van-thai-prod.mjs
 */
...
async function main() {
    if (!SESSION_TOKEN) {
        console.error('❌ Cần SESSION_TOKEN');
        console.log('   Mở erp.motnha.vn → F12 → Application → Cookies → next-auth.session-token');
        console.log('   Chạy: SESSION_TOKEN=xxx node scripts/upload-van-thai-prod.mjs');
        process.exit(1);
    }

    console.log(`\n🚀 Upload ${PRODUCTS.length} ảnh Ván Thái → ${PROD_URL}/api/upload\n`);
    let uploaded = 0, failed = 0;

    for (const p of PRODUCTS) {
        // 1. Tải ảnh từ truongsonmelamine.com
        let buf;
        try {
            ...
            console.error(`  ✗ Tải ${p.code}: ${e.message}`);
            failed++;
            continue;
        }

        // 2. Upload lên production qua API
        let r2Url;
        try {
            const filename = `van-thai-${p.code.replace(/\s+/g, '-').toLowerCase()}.jpg`;
            const blob = new Blob([buf], { type: 'image/jpeg' });
            const form = new FormData();
            form.append('file', blob, filename);
            form.append('type', 'products');

            const uploadRes = await fetch(`${PROD_URL}/api/upload`, {
                method: 'POST',
                body: form,
            ...
            if (!uploadRes.ok) {
                ...
                throw new Error(`${uploadRes.status} — ${err}`);
            }
            const json = await uploadRes.json();
            r2Url = json.url;
        } catch (e) {
            console.error(`  ✗ Upload ${p.code}: ${e.message}`);
            failed++;
            continue;
        }
        ...
        await prisma.product.updateMany({
            where: { name: `Ván Melamin Thái Lan ${p.code}` },
            data: { image: r2Url },
        });
        uploaded++;
        process.stdout.write(`  ↑ ${p.code} → ${r2Url}\n`);
    }

    console.log(`\n────────────────────────────`);
    console.log(`✅ Upload: ${uploaded}, lỗi: ${failed}`);
}
