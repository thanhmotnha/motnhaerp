/**
 * Upload ảnh Ván Thái lên Cloudflare R2 + cập nhật DB
 *
 * Yêu cầu: thêm R2_* vào .env trước khi chạy
 * Run: node scripts/upload-van-thai-r2.mjs
 */
...
if (!R2_ACCOUNT_ID || !R2_ACCESS_KEY_ID || !R2_SECRET_ACCESS_KEY || !R2_BUCKET_NAME) {
    console.error('❌ Thiếu R2_* trong .env');
    process.exit(1);
}
...
const PUBLIC_BASE = (R2_PUBLIC_URL || `https://${R2_BUCKET_NAME}.r2.dev`).replace(/\/$/, '');
...
async function main() {
    console.log(`\n🚀 Upload ${PRODUCTS.length} ảnh Ván Thái → R2 bucket: ${R2_BUCKET_NAME}\n`);
    let uploaded = 0, skipped = 0, failed = 0;

    for (const p of PRODUCTS) {
        const slug = 'van-thai-' + p.code.replace(/\s+/g, '-').toLowerCase();
        const key = `products/${slug}.jpg`;
        const r2Url = `${PUBLIC_BASE}/${key}`;

        // Tải ảnh từ truongsonmelamine.com
        let buf;
        try {
            ...
            console.error(`  ✗ Tải ảnh ${p.code}: ${e.message}`);
            failed++;
            continue;
        }
        ...
        try {
            ...
            console.error(`  ✗ Upload R2 ${p.code}: ${e.message}`);
            failed++;
            continue;
        }
    ...
    const sample = await prisma.product.findFirst({
        where: { name: { contains: 'Ván Melamin Thái Lan' } },
