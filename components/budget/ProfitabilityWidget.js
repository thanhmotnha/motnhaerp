...
export default function ProfitabilityWidget({ projectId }) {
    ...
    const cards = [
        ...
        { label: 'LN Mục tiêu (A-B)', value: data.targetProfit, color: data.targetProfit >= 0 ? '#10b981' : '#ef4444', icon: '🎯', sub: `${data.targetMargin}%` },
        { label: 'LN Tạm tính', value: data.estimatedProfit, color: data.estimatedProfit >= 0 ? '#10b981' : '#ef4444', icon: '📊', sub: `${data.estimatedMargin}%` },
    ];
