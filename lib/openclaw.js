...
/**
 * Gửi cho KHÁCH HÀNG → Zalo (theo số điện thoại)
 */
export async function sendToCustomer({ phone, toName, content, event }) {
    return sendViaOpenClaw({ channel: 'zalo', to: phone, toName, content, event });
}

/**
 * Gửi cho NHÂN VIÊN → Lark (theo larkId cá nhân hoặc chat group id)
 */
export async function sendToEmployee({ larkId, toName, content, event }) {
    return sendViaOpenClaw({ channel: 'lark', to: larkId, toName, content, event });
}
