/**
 * Fix Ván Thái: cập nhật image → URL truongsonmelamine.com (luôn có sẵn)
 *               + bỏ dấu ngoặc trong tên SP
 * Run: node scripts/fix-van-thai-images.mjs
 */
...
async function main() {
    ...
    for (const p of PRODUCTS) {
        // Tên cũ có ngoặc: "Ván Melamin Thái Lan (031)"
        const oldName = `Ván Melamin Thái Lan (${p.code})`;
        const newName = `Ván Melamin Thái Lan ${p.code}`;
        const res = await prisma.product.updateMany({
            where: { name: oldName },
            data: { image: p.img, name: newName },
        });
