...
export const POST = withAuth(async (_req, _ctx, session) => {
    if (session.user.role !== 'giam_doc') {
        return NextResponse.json({ error: 'Chỉ Giám đốc mới có quyền' }, { status: 403 });
    }

    // Ensure category exists
    let category = await prisma.productCategory.findFirst({ where: { name: 'Ván Thái' } });
    if (!category) {
        category = await prisma.productCategory.create({
            data: { name: 'Ván Thái', slug: 'van-thai', order: 50 },
        });
    }

    // Get existing product names to skip duplicates
    const existing = await prisma.product.findMany({
        where: { category: 'Ván Thái', deletedAt: null },
        select: { color: true },
    });
    const existingCodes = new Set(existing.map(p => p.color));

    // Get max SP code number
    const lastSP = await prisma.product.findFirst({
        where: { code: { startsWith: 'SP-' } },
        orderBy: { code: 'desc' },
        select: { code: true },
    });
    let nextNum = lastSP ? parseInt(lastSP.code.replace(/\D/g, '')) + 1 : 1;
    ...
    for (const p of PRODUCTS) {
        ...
        const spCode = `SP-${String(nextNum++).padStart(4, '0')}`;
        try {
            await prisma.product.create({
                data: {
                    code: spCode,
                    name: `Ván Melamin Thái Lan ${p.code}`,
                    unit: 'tờ',
                    category: 'Ván Thái',
                    categoryId: category.id,
                    color: p.code,
                    image: p.img,
                    brand: 'Melamine Thái Lan',
                    origin: 'Thái Lan',
                    status: 'Đang bán',
            ...
            // skip duplicate code errors
            nextNum++;
        }
    }

    return NextResponse.json({ ok: true, created, skipped, total: PRODUCTS.length });
});
